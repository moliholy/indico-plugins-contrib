...
None of the five is invisible over GET. Every one of them can be read
...
The ones that can only be read as HTML today are event layout and features and
...
videoconferences and offline copies are the entities served
...
well, with nothing behind its link but the ZIP file itself.
