...
def test_session_type_details(dummy_event, poster_session_type, token_headers, test_client):
    resp = test_client.get(
        f'/api/v1/events/{dummy_event.id}/session-types/{poster_session_type.id}', headers=token_headers
    )
    assert resp.status_code == 200
    assert resp.json == {
        'id': poster_session_type.id,
...
def test_session_type_list_is_sorted_by_name(
    dummy_event, poster_session_type, create_session_type, token_headers, test_client
):
...
def test_session_type_denied_without_event_access(db, dummy_event, poster_session_type, outsider_headers, test_client):
    dummy_event.protection_mode = ProtectionMode.protected
    db.session.flush()
    resp = test_client.get(
        f'/api/v1/events/{dummy_event.id}/session-types/{poster_session_type.id}', headers=outsider_headers
    )
...
def test_session_type_of_another_event_is_not_found(db, poster_session_type, create_event, token_headers, test_client):
    other = create_event()
    db.session.flush()
    resp = test_client.get(f'/api/v1/events/{other.id}/session-types/{poster_session_type.id}', headers=token_headers)
    assert resp.status_code == 404


def test_session_type_matches_the_session_it_is_used_by(
    db, dummy_event, dummy_session, poster_session_type, token_headers, test_client
):
    ...
    session = test_client.get(
        f'/api/v1/events/{dummy_event.id}/sessions/{dummy_session.id}', headers=token_headers
    ).json
    new = test_client.get(
        f'/api/v1/events/{dummy_event.id}/session-types/{poster_session_type.id}', headers=token_headers
    ).json
