...
def test_paper_details(dummy_event, dummy_contribution, dummy_paper_revision, dummy_user, token_headers, test_client):
    resp = test_client.get(
        f'/api/v1/events/{dummy_event.id}/contributions/{dummy_contribution.id}/paper', headers=token_headers
    )
...
def test_paper_list_skips_contributions_without_paper(
    dummy_event, dummy_contribution, dummy_paper_revision, create_contribution, token_headers, test_client
):
...
def test_paper_of_someone_else_is_forbidden(
    dummy_event, dummy_contribution, dummy_paper_revision, outsider_headers, test_client
):
    resp = test_client.get(
        f'/api/v1/events/{dummy_event.id}/contributions/{dummy_contribution.id}/paper', headers=outsider_headers
    )
...
def test_paper_manager_sees_every_paper(
    db, dummy_event, dummy_contribution, dummy_paper_revision, outsider, outsider_headers, test_client
):
...
def test_papers_require_the_feature(
    db, dummy_event, dummy_contribution, dummy_paper_revision, token_headers, test_client
):
...
def test_judged_paper(
    db, dummy_event, dummy_contribution, dummy_paper_revision, dummy_user, token_headers, test_client
):
...
def test_paper_matches_current_api(
    dummy_event,
    dummy_contribution,
    dummy_paper_revision,
    dummy_paper_file,
...
def test_judged_paper_matches_current_api(
    dummy_event, dummy_contribution, dummy_paper_file, paper_manager, token_headers, test_client, indico_api, same_json
):
    current = indico_api(f'/event/{dummy_event.id}/manage/papers/assignment-list/export-json')
    paper = next(p for p in current['papers'] if p['contribution']['id'] == dummy_contribution.id)
    new = test_client.get(
...
def test_paper_list_matches_current_api(
    dummy_event,
    dummy_contribution,
    dummy_paper_revision,
    dummy_paper_file,
