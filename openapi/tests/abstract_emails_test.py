...
def acceptance_template(create_email_template):
    return create_email_template('Acceptance', reply_to_address='chair@example.com',
                                 extra_cc_emails=['records@example.com'], include_authors=True,
                                 include_submitter=True, include_coauthors=False, stop_on_match=True)


@pytest.fixture
def sent_email(db, dummy_abstract, acceptance_template):
    entry = AbstractEmailLogEntry(abstract=dummy_abstract, email_template=acceptance_template,
                                  sent_dt=datetime(2026, 9, 1, 8, 0, tzinfo=UTC),
                                  recipients=['doe@example.com', 'records@example.com'],
                                  subject='Your abstract was accepted', body='Dear John Doe,',
...
def test_abstract_email_template_details(dummy_event, acceptance_template, token_headers, test_client):
    resp = test_client.get(f'/api/v1/events/{dummy_event.id}/abstract-email-templates/{acceptance_template.id}',
                           headers=token_headers)
    assert resp.status_code == 200
    assert resp.json == {'id': acceptance_template.id, 'event_id': dummy_event.id, 'position': 1,
                         'title': 'Acceptance', 'subject': 'Your abstract was accepted',
                         'body': 'Dear {abstract_submitter},', 'reply_to_address': 'chair@example.com',
                         'extra_cc_emails': ['records@example.com'], 'include_submitter': True,
                         'include_authors': True, 'include_coauthors': False, 'stop_on_match': True,
                         'rules': [{'state': [1]}]}


@pytest.mark.usefixtures('abstract_manager')
def test_abstract_email_template_list_follows_positions(dummy_event, acceptance_template, create_email_template,
...
@pytest.mark.usefixtures('abstract_manager')
def test_abstract_email_templates_need_the_abstracts_feature(db, dummy_event, acceptance_template, token_headers,
                                                             test_client):
...
@pytest.mark.usefixtures('abstract_manager')
def test_abstract_email_template_of_another_event_is_not_found(db, acceptance_template, create_event, dummy_user,
                                                               token_headers, test_client):
    other = create_event()
    other.update_principal(dummy_user, permissions={'abstracts'})
    set_feature_enabled(other, 'abstracts', True)
    db.session.flush()
    resp = test_client.get(f'/api/v1/events/{other.id}/abstract-email-templates/{acceptance_template.id}',
                           headers=token_headers)
    assert resp.status_code == 404


@pytest.mark.usefixtures('abstract_manager')
def test_abstract_email_details(dummy_event, dummy_abstract, acceptance_template, sent_email, dummy_user,
                                token_headers, test_client):
    resp = test_client.get(f'/api/v1/events/{dummy_event.id}/abstracts/{dummy_abstract.id}/emails/{sent_email.id}',
                           headers=token_headers)
    assert resp.status_code == 200
    assert resp.json == {'id': sent_email.id, 'abstract_id': dummy_abstract.id,
                         'email_template_id': acceptance_template.id, 'template_title': 'Acceptance',
                         'sent_dt': '2026-09-01T08:00:00+00:00', 'sent_by': None,
                         'recipients': ['doe@example.com', 'records@example.com'],
                         'subject': 'Your abstract was accepted', 'body': 'Dear John Doe,'}


@pytest.mark.usefixtures('abstract_manager')
def test_abstract_email_list_is_oldest_first(db, dummy_event, dummy_abstract, sent_email, dummy_user, token_headers,
                                             test_client):
    later = AbstractEmailLogEntry(abstract=dummy_abstract, user=dummy_user,
                                  sent_dt=datetime(2026, 9, 2, 8, 0, tzinfo=UTC), recipients=['doe@example.com'],
                                  subject='Reminder', body='Dear John Doe,', data={})
    db.session.flush()
    resp = test_client.get(f'/api/v1/events/{dummy_event.id}/abstracts/{dummy_abstract.id}/emails',
                           headers=token_headers)
    assert resp.status_code == 200
...
def test_abstract_emails_need_judging_rights(dummy_event, dummy_abstract, dummy_user, sent_email, token_headers,
                                            test_client):
...
@pytest.mark.usefixtures('abstract_manager')
def test_abstract_email_of_another_abstract_is_not_found(db, dummy_event, dummy_abstract, dummy_user, sent_email,
                                                         create_abstract, token_headers, test_client):
    other = create_abstract(dummy_event, 'Another abstract', friendly_id=315, submitter=dummy_user)
    db.session.flush()
    resp = test_client.get(f'/api/v1/events/{dummy_event.id}/abstracts/{other.id}/emails/{sent_email.id}',
                           headers=token_headers)
    assert resp.status_code == 404
