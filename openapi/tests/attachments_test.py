...
def dummy_link(db, dummy_user, dummy_contribution):
    ...
    link = Attachment(
        folder=folder, user=dummy_user, type=AttachmentType.link, title='Indico', link_url='https://getindico.io'
    )
    db.session.flush()
    return link
...
def test_contribution_attachment_list(dummy_event, dummy_contribution, dummy_link, token_headers, test_client):
    resp = test_client.get(
        f'/api/v1/events/{dummy_event.id}/contributions/{dummy_contribution.id}/attachments', headers=token_headers
    )
    assert resp.status_code == 200
...
def test_event_attachment_not_listed_for_contribution(
    dummy_event, dummy_contribution, dummy_attachment, token_headers, test_client
):
    resp = test_client.get(
        f'/api/v1/events/{dummy_event.id}/contributions/{dummy_contribution.id}/attachments', headers=token_headers
    )
...
def test_attachment_denied_without_event_access(db, dummy_event, dummy_attachment, outsider_headers, test_client):
    ...
    db.session.flush()
    resp = test_client.get(
        f'/api/v1/events/{dummy_event.id}/attachments/{dummy_attachment.id}', headers=outsider_headers
    )
...
def flatten(folders):
    return [{**attachment, 'folder': folder} for folder in folders for attachment in folder['attachments']]


def test_event_attachment_matches_current_api(
    dummy_event, dummy_attachment, token_headers, test_client, indico_api, same_json, rename_keys
):
    current = flatten(indico_api(f'/export/attachments/{dummy_event.id}.json')['results']['folders'])[0]
    new = test_client.get(
        f'/api/v1/events/{dummy_event.id}/attachments/{dummy_attachment.id}', headers=token_headers
    ).json
    same_json(new, current, same=FILE_FIELDS, renamed={'folder': ('folder', rename_keys(FOLDER_KEYS))})


def test_event_attachment_list_matches_current_api(
    dummy_event,
    dummy_user,
    dummy_attachment,
    create_attachment,
...
def test_link_attachment_matches_current_api(
    dummy_event, dummy_contribution, dummy_link, token_headers, test_client, indico_api, same_json, rename_keys
):
    ...
    new = test_client.get(
        f'/api/v1/events/{dummy_event.id}/contributions/{dummy_contribution.id}/attachments/{dummy_link.id}',
