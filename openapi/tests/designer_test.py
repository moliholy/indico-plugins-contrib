...
def test_designer_template_details(dummy_event, dummy_designer_template, token_headers, test_client):
    resp = test_client.get(f'/api/v1/events/{dummy_event.id}/designer-templates/{dummy_designer_template.id}',
...
@pytest.mark.usefixtures('event_manager')
def test_designer_template_serves_its_images(dummy_event, dummy_designer_template, create_image, token_headers,
                                             test_client):
...
@pytest.mark.usefixtures('event_manager')
def test_designer_template_serves_its_background(db, dummy_event, dummy_designer_template, create_image, token_headers,
                                                 test_client):
...
@pytest.mark.usefixtures('event_manager')
def test_designer_template_list_includes_inherited_templates(dummy_event, dummy_designer_template,
                                                             category_designer_template, token_headers, test_client):
...
@pytest.mark.usefixtures('event_manager')
def test_designer_template_list_honours_the_filter_signal(dummy_event, dummy_designer_template,
                                                          category_designer_template, token_headers, test_client):
...
@pytest.mark.usefixtures('event_manager')
def test_template_of_an_unrelated_event_is_not_found(dummy_event, create_event, create_dummy_designer_template,
...
@pytest.mark.usefixtures('event_manager')
def test_designer_template_matches_current_api(dummy_event, dummy_designer_template, create_image, db, token_headers,
    ...
    new = test_client.get(f'/api/v1/events/{dummy_event.id}/designer-templates/{dummy_designer_template.id}',
                          headers=token_headers).json
    same_json(new, theirs, same=('title', 'data', 'background_url'),
              renamed={'id': ('backside_template_id', None), 'images': ('images', as_image_map)})


@pytest.mark.usefixtures('event_manager')
def test_designer_template_without_images_matches_current_api(dummy_event, dummy_designer_template, token_headers,
                                                              test_client, indico_api, same_json):
    current = indico_api(f'/event/{dummy_event.id}/manage/designer/{dummy_designer_template.id}/data')
    theirs = current['template'] | {'backside_template_id': current['backside_template_id']}
    new = test_client.get(f'/api/v1/events/{dummy_event.id}/designer-templates/{dummy_designer_template.id}',
