...
def test_session_list_hides_protected_sessions(
    db, dummy_event, dummy_session, create_session, outsider_headers, test_client
):
    dummy_session.protection_mode = ProtectionMode.protected
...
SESSION_KEYS = {
    'id': ('db_id', None),
    'is_poster': ('isPoster', None),
    'text_color': ('textColor', None),
    'background_color': ('color', None),
    'venue_name': ('location', None),
...
def as_blocks(as_legacy_date):
    def convert(blocks):
        return [
            {
                BLOCK_KEYS.get(key, key): as_legacy_date(value) if key.endswith('_dt') else value
...
def test_session_matches_current_api(
    dummy_event, dummy_session, dummy_session_block, token_headers, test_client, indico_api, as_legacy_date, same_json
):
    ...
    same_json(
        new,
        as_sessions(slots)[dummy_session.id],
        same=SESSION_FIELDS,
        renamed={**SESSION_KEYS, 'blocks': ('blocks', as_blocks(as_legacy_date))},
    )


def test_session_list_matches_current_api(
    dummy_event,
    dummy_session,
    dummy_session_block,
    create_session,
