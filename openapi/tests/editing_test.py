...
EDITABLE_FIELDS = ('id', 'type', 'contribution', 'review_conditions_valid', 'editing_enabled',
                   'has_published_revision', 'last_update_dt')
...
FILE_TYPE_FIELDS = ('id', 'name', 'extensions', 'allow_multiple_files', 'required', 'publishable',
                    'filename_template')
...
def _as_editing_user(user):
    return user and {'id': user['id'], 'identifier': user['identifier'], 'full_name': user['full_name'],
                     'anonymous': False}
...
def as_files(current):
    return [{'id': file['id'], 'uuid': file['uuid'], 'filename': file['filename'], 'size': file['size'],
             'content_type': file['content_type'], 'file_type_id': file['file_type'],
             'download_url': file['download_url']}
            for file in current['files']]


EDITABLE_MAPPINGS = {'same': EDITABLE_FIELDS,
                     'derived': {'state': as_state_name, 'editor': as_editor, 'revision_count': as_revision_count}}
REVISION_MAPPINGS = {'same': REVISION_FIELDS,
...
def create_revision(db, editable, dummy_user):
    def _create(type=RevisionType.ready_for_review, user=None, comment='', is_undone=False):
        revision = EditingRevision(editable=editable, user=(user or dummy_user), type=type, comment=comment,
                                   is_undone=is_undone, created_dt=now_utc())
...
def file_type(db, dummy_event):
    file_type = EditingFileType(event=dummy_event, type=EditableType.paper, name='Source', extensions=['tex'],
                                required=True, publishable=False, filename_template='paper-{code}')
    db.session.add(file_type)
    db.session.flush()
    return file_type


def test_editable_is_served_to_the_submitter(dummy_event, dummy_contribution, editable, revision, token_headers,
                                             test_client):
    resp = test_client.get(f'/api/v1/events/{dummy_event.id}/contributions/{dummy_contribution.id}'
                           f'/editables/paper', headers=token_headers)
...
def test_editable_of_an_unknown_type_is_not_found(dummy_event, dummy_contribution, editable, token_headers,
                                                  test_client):
    resp = test_client.get(f'/api/v1/events/{dummy_event.id}/contributions/{dummy_contribution.id}'
                           f'/editables/slides', headers=token_headers)
    assert resp.status_code == 404


def test_editable_is_not_served_to_an_outsider(dummy_event, dummy_contribution, editable, outsider_headers,
                                               test_client):
    resp = test_client.get(f'/api/v1/events/{dummy_event.id}/contributions/{dummy_contribution.id}'
                           f'/editables/paper', headers=outsider_headers)
    assert resp.status_code == 403


def test_undone_revision_is_hidden_from_the_submitter(dummy_event, dummy_contribution, editable, create_revision,
...
def test_undone_revision_is_served_to_the_editing_team(db, dummy_event, dummy_contribution, dummy_user, editable,
                                                       create_revision, token_headers, test_client):
...
def test_editor_name_is_hidden_while_the_team_is_anonymous(db, dummy_event, dummy_contribution, editable, editor,
                                                           revision, token_headers, test_client):
...
def test_editor_name_is_served_to_the_editing_team(db, dummy_event, dummy_contribution, dummy_user, editable, editor,
                                                   revision, token_headers, test_client):
    editable.editor = editor
    editable_type_settings[EditableType.paper].set(dummy_event, 'anonymous_team', True)
    dummy_event.update_principal(dummy_user, permissions={'paper_editing'})
...
def test_editable_list_leaves_out_what_the_caller_cannot_see(dummy_event, editable, revision, outsider_headers,
                                                             test_client):
...
def test_revision_comments_are_served(db, dummy_event, dummy_contribution, dummy_user, editable, revision,
                                      token_headers, test_client):
    comment = EditingRevisionComment(revision=revision, user=dummy_user, text='Looks good')
    db.session.add(comment)
    db.session.flush()
    resp = test_client.get(f'/api/v1/events/{dummy_event.id}/contributions/{dummy_contribution.id}'
                           f'/editables/paper/revisions/{revision.id}/comments', headers=token_headers)
    assert resp.status_code == 200
...
def test_internal_comment_is_hidden_from_the_submitter(db, dummy_event, dummy_contribution, editable, editor,
                                                       revision, token_headers, test_client):
...
def test_internal_comment_is_served_to_the_editing_team(db, dummy_event, dummy_contribution, dummy_user, editable,
                                                        revision, token_headers, test_client):
...
def test_comments_of_an_unknown_revision_are_not_found(dummy_event, dummy_contribution, editable, revision,
                                                       token_headers, test_client):
    resp = test_client.get(f'/api/v1/events/{dummy_event.id}/contributions/{dummy_contribution.id}'
                           f'/editables/paper/revisions/{revision.id + 1000}/comments', headers=token_headers)
    assert resp.status_code == 404
...
def test_editing_endpoints_need_the_feature(db, dummy_event, dummy_contribution, editable, editing_tag,
                                            token_headers, test_client):
    set_feature_enabled(dummy_event, 'editing', False)
    db.session.flush()
    for path in (f'/events/{dummy_event.id}/editables',
...
def test_review_conditions_are_served_to_a_manager(db, dummy_event, file_type, event_manager, token_headers,
                                                   test_client):
...
def test_editable_matches_the_timeline(dummy_event, dummy_contribution, editable, revision, revision_file,
                                       indico_api, same_json, same_json_list):
...
def test_comments_match_the_timeline(db, dummy_event, dummy_contribution, dummy_user, editable, revision,
                                     indico_api, same_json_list):
    db.session.add(EditingRevisionComment(revision=revision, user=dummy_user, text='Looks good'))
    db.session.flush()
    ours = indico_api(f'/api/v1/events/{dummy_event.id}/contributions/{dummy_contribution.id}'
