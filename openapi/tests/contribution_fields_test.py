...
def create_contribution_field(db, dummy_event):
    def _create(title, field_type='text', field_data=None, **kwargs):
        field = ContributionField(event=dummy_event, title=title, field_type=field_type,
                                  field_data=field_data or {}, **kwargs)
...
def summary_field(create_contribution_field):
    return create_contribution_field('Summary', description='A few lines.', is_required=True,
...
def test_contribution_field_details(dummy_event, summary_field, token_headers, test_client):
    resp = test_client.get(f'/api/v1/events/{dummy_event.id}/contribution-fields/{summary_field.id}',
                           headers=token_headers)
    assert resp.status_code == 200
    assert resp.json == {'id': summary_field.id, 'event_id': dummy_event.id, 'position': 1, 'title': 'Summary',
                         'description': 'A few lines.', 'is_required': True, 'is_active': True,
                         'is_user_editable': True, 'visibility': 'public', 'field_type': 'text',
                         'field_data': {'max_length': 500, 'max_words': 0, 'multiline': True}}


def test_contribution_field_list_follows_positions(dummy_event, summary_field, create_contribution_field,
                                                   token_headers, test_client):
    choice = create_contribution_field('Format', field_type='single_choice',
                                       field_data={'display_type': 'select', 'options': []})
...
def test_restricted_contribution_fields_are_manager_only(dummy_event, summary_field, internal_field, retired_field,
...
def test_managers_see_every_contribution_field(dummy_event, summary_field, internal_field, retired_field,
                                               token_headers, test_client):
    url = f'/api/v1/events/{dummy_event.id}/contribution-fields'
...
def test_contribution_field_denied_without_event_access(db, dummy_event, summary_field, outsider_headers,
                                                        test_client):
...
def test_contribution_field_of_another_event_is_not_found(db, summary_field, create_event, token_headers,
                                                          test_client):
    other = create_event()
    db.session.flush()
...
FIELD_FIELDS = ('id', 'position', 'title', 'description', 'is_required', 'is_active', 'is_user_editable',
                'visibility', 'field_type', 'field_data')


@pytest.mark.usefixtures('event_manager')
def test_contribution_field_list_matches_current_api(dummy_event, summary_field, internal_field, retired_field,
