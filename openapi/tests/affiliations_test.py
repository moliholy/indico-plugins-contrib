...
def test_affiliation_details(dummy_affiliation, token_headers, test_client):
    ...
    assert resp.json == {'id': dummy_affiliation.id, 'name': 'ACME', 'code': 'ACM', 'alt_names': ['Acme Inc'],
                         'street': '1 Main street', 'postcode': '1211', 'city': 'Geneva', 'country_code': 'CH',
                         'country_name': 'Switzerland', 'meta': {'source': 'test'}}
...
def test_affiliations_are_served_to_any_caller(dummy_affiliation, outsider_headers, test_client):
    ...
    assert test_client.get(f'/api/v1/affiliations/{dummy_affiliation.id}',
                           headers=outsider_headers).status_code == 200


AFFILIATION_FIELDS = ('id', 'name', 'code', 'alt_names', 'street', 'postcode', 'city', 'country_code', 'country_name',
                      'meta')


def test_affiliation_list_matches_current_api(dummy_affiliation, create_affiliation, admin_headers, test_client,
