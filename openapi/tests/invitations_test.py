...
def create_invitation(db, dummy_regform):
    def _create(first_name, last_name, email, regform=None, **kwargs):
        invitation = RegistrationInvitation(
            ...
            last_name=last_name,
            email=email,
            affiliation=kwargs.pop('affiliation', 'ACME'),
            **kwargs,
        )
...
def test_invitation_details(dummy_event, dummy_regform, dummy_invitation, token_headers, test_client):
    resp = test_client.get(
        f'/api/v1/events/{dummy_event.id}/registration-forms/{dummy_regform.id}/invitations/{dummy_invitation.id}',
        headers=token_headers,
    )
    assert resp.status_code == 200
    assert resp.json == {
        'id': dummy_invitation.id,
...
@pytest.mark.usefixtures('event_manager')
def test_accepted_invitation_points_at_the_registration(
    db, dummy_event, dummy_regform, dummy_reg, dummy_invitation, token_headers, test_client
):
...
@pytest.mark.usefixtures('event_manager')
def test_invitation_list_is_sorted_by_name(
    dummy_event, dummy_regform, dummy_invitation, create_invitation, token_headers, test_client
):
    ...
    resp = test_client.get(
        f'/api/v1/events/{dummy_event.id}/registration-forms/{dummy_regform.id}/invitations', headers=token_headers
    )
    assert resp.status_code == 200
    assert [inv['id'] for inv in resp.json['results']] == [dummy_invitation.id, alan.id, grace.id]
    assert resp.json['count'] == 3
    resp = test_client.get(
        f'/api/v1/events/{dummy_event.id}/registration-forms/{dummy_regform.id}/invitations?state=declined',
        headers=token_headers,
    )
...
def test_registration_managers_can_see_invitations(
    dummy_event, dummy_regform, dummy_invitation, token_headers, test_client
):
    resp = test_client.get(
        f'/api/v1/events/{dummy_event.id}/registration-forms/{dummy_regform.id}/invitations', headers=token_headers
    )
...
@pytest.mark.usefixtures('event_manager')
def test_invitations_need_the_registration_feature(
    db, dummy_event, dummy_regform, dummy_invitation, token_headers, test_client
):
...
@pytest.mark.usefixtures('event_manager')
def test_invitations_of_a_deleted_form_are_not_found(
    db, dummy_event, dummy_regform, dummy_invitation, token_headers, test_client
):
...
@pytest.mark.usefixtures('event_manager')
def test_invitation_of_another_form_is_not_found(
    dummy_event, dummy_regform, create_regform, create_invitation, token_headers, test_client
):
    other = create_regform(dummy_event, title='Other')
    stranger = create_invitation('Linus', 'Torvalds', 'linus@example.com', regform=other)
    resp = test_client.get(
        f'/api/v1/events/{dummy_event.id}/registration-forms/{dummy_regform.id}/invitations/{stranger.id}',
        headers=token_headers,
    )
    assert resp.status_code == 404
