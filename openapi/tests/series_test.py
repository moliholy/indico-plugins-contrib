...
@pytest.mark.usefixtures('event_manager')
def test_event_series_lists_its_events_in_order(db, dummy_event, create_event, create_series, token_headers,
                                               test_client):
    later = create_event(1, start_dt=dummy_event.start_dt + timedelta(days=1),
                         end_dt=dummy_event.end_dt + timedelta(days=1), creator_has_privileges=True)
    earlier = create_event(2, start_dt=dummy_event.start_dt - timedelta(days=1),
                           end_dt=dummy_event.end_dt - timedelta(days=1), creator_has_privileges=True)
...
@pytest.mark.usefixtures('event_manager')
def test_event_series_list_only_holds_series_the_caller_manages(dummy_event, create_event, create_series, token_headers,
                                                                test_client):
...
@pytest.mark.usefixtures('event_manager')
def test_a_series_holding_an_unmanaged_event_is_forbidden(dummy_event, create_event, create_series, token_headers,
                                                          test_client):
...
@pytest.mark.usefixtures('event_manager')
def test_event_series_matches_current_api(dummy_event, create_event, create_series, token_headers, test_client,
...
@pytest.mark.usefixtures('event_manager')
def test_event_series_list_matches_current_api(dummy_event, create_event, create_series, token_headers, test_client,
