...
def test_payment_details(dummy_event, dummy_reg, dummy_payment, token_headers, test_client):
    ...
    assert resp.json == {'id': dummy_payment.id, 'registration_id': dummy_reg.id, 'status': 'successful',
                         'amount': '10.50', 'currency': 'USD', 'provider': '_manual',
...
@pytest.mark.usefixtures('event_manager')
def test_payment_list_filters_by_registration(dummy_event, dummy_regform, dummy_payment, create_payment,
                                              create_registration, create_user, db, token_headers, test_client):
    ...
    resp = test_client.get(f'/api/v1/events/{dummy_event.id}/payments?registration_id={other_reg.id}',
...
def test_payments_are_manager_only_even_for_the_registrant(dummy_event, dummy_reg, dummy_payment, dummy_user,
...
@pytest.mark.usefixtures('event_manager')
def test_payment_of_a_deleted_registration_is_not_served(db, dummy_event, dummy_reg, dummy_payment, token_headers,
                                                        test_client):
...
def test_payment_of_another_event_is_not_found(db, dummy_payment, create_event, dummy_user, token_headers,
                                               test_client):
    other = create_event()
