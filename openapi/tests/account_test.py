...
SETTING_FIELDS = {'lang', 'force_language', 'timezone', 'force_timezone', 'show_future_events', 'show_past_events',
                  'name_format', 'use_previewer_pdf', 'add_ical_alerts', 'add_ical_alerts_mins',
                  'use_markdown_for_minutes', 'synced_fields', 'suggest_categories', 'mastodon_server_url',
...
def test_settings_follow_what_the_user_saved(db, dummy_user, token_headers, test_client):
    dummy_user.settings.set_multi({'timezone': 'Europe/Zurich', 'force_timezone': True, 'add_ical_alerts_mins': 30,
...
def test_emails_list_the_primary_one_first(db, dummy_user, token_headers, test_client):
    ...
    assert resp.status_code == 200
    assert resp.json['results'] == [{'email': dummy_user.email, 'is_primary': True},
...
def test_data_export_request_is_served(export_request, token_headers, test_client):
    ...
    assert resp.status_code == 200
    assert resp.json == {'state': 'running', 'requested_dt': export_request.requested_dt.isoformat(),
                         'selected_options': ['personal_data', 'registrations'], 'include_files': True,
                         'max_size_exceeded': False, 'url': None}
...
def test_account_endpoints_never_answer_about_another_user(export_request, path, status, outsider_headers,
                                                           test_client):
