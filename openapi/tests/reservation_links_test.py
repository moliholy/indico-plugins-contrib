...
def booking_of_every_type(db, create_reservation, dummy_event, dummy_contribution, dummy_session_block):
    today = date.today()
    reservation = create_reservation(start_dt=datetime.combine(today, time(8, 30)),
                                     end_dt=datetime.combine(today + timedelta(days=2), time(17, 30)),
                                     repeat_frequency=RepeatFrequency.DAY)
    for occurrence, obj in zip(reservation.occurrences, (dummy_event, dummy_contribution, dummy_session_block),
                              strict=True):
...
def test_links_of_every_type(booking_of_every_type, dummy_event, dummy_contribution, dummy_session_block,
    ...
    assert [link['title'] for link in links] == [dummy_event.title, dummy_contribution.title,
...
def link_derived(ours):
    ...
    return {'event_id': lambda current: _id_in(current['object']['event_url'], r'/event/(\d+)'),
...
def test_reservation_link_list_matches_current_api(booking_of_every_type, token_headers, test_client,
    ...
    same_json_list(new['results'], current, same=LINK_FIELDS, renamed=LINK_KEYS,
                   derived=link_derived(new['results']))
