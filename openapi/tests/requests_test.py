...
def test_service_request_details(dummy_event, dummy_service_request, dummy_user, token_headers, test_client):
    resp = test_client.get(f'/api/v1/events/{dummy_event.id}/requests/{dummy_service_request.id}',
                           headers=token_headers)
    assert resp.status_code == 200
    assert resp.json == {'id': dummy_service_request.id, 'event_id': dummy_event.id, 'type': 'cats',
                         'state': 'pending', 'data': {'how_many': 3}, 'comment': None,
                         'created_dt': '2026-09-01T08:00:00+00:00', 'processed_dt': None, 'processed_by': None,
                         'created_by': {'id': dummy_user.id, 'identifier': dummy_user.identifier,
                                        'full_name': dummy_user.full_name, 'email': dummy_user.email}}


@pytest.mark.usefixtures('event_manager', 'definitions')
def test_service_request_serves_who_processed_it(db, dummy_event, dummy_service_request, outsider, token_headers,
                                                test_client):
...
@pytest.mark.usefixtures('event_manager', 'definitions')
def test_service_request_list_is_newest_first(dummy_event, dummy_service_request, create_service_request,
...
@pytest.mark.usefixtures('event_manager', 'definitions')
def test_service_request_list_filters_by_type(dummy_event, dummy_service_request, create_service_request,
                                              token_headers, test_client):
...
@pytest.mark.usefixtures('event_manager', 'definitions')
def test_service_request_list_filters_by_state(dummy_event, dummy_service_request, create_service_request,
                                               token_headers, test_client):
...
@pytest.mark.usefixtures('event_manager', 'definitions')
def test_request_types_without_a_definition_are_not_served(dummy_event, create_service_request, token_headers,
                                                           test_client):
...
def test_request_managers_see_only_the_types_they_manage(definitions, dummy_event, dummy_service_request,
                                                        create_service_request, dummy_user, token_headers,
                                                        test_client):
...
@pytest.mark.usefixtures('definitions')
def test_service_request_of_another_event_is_not_found(db, dummy_service_request, create_event, dummy_user,
                                                      token_headers, test_client):
    other = create_event()
