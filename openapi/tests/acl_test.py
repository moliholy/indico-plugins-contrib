...
def test_event_acl_lists_every_named_principal(dummy_event, event_acl, dummy_user, outsider, dummy_group,
    ...
    assert entries[f'User:{dummy_user.id}'] == {'type': 'user', 'identifier': f'User:{dummy_user.id}',
                                                'name': dummy_user.name, 'read_access': False, 'full_access': True,
                                                'permissions': []}
...
def test_acl_leaves_out_permissions_the_object_does_not_define(db, dummy_event, dummy_user, token_headers,
                                                               test_client):
...
def test_session_acl(db, dummy_event, dummy_session, outsider, event_manager, token_headers, test_client):
    ...
    assert resp.json['results'] == [{'type': 'user', 'identifier': f'User:{outsider.id}', 'name': outsider.name,
                                     'read_access': False, 'full_access': False, 'permissions': ['coordinate']}]
...
def test_contribution_acl(db, dummy_event, dummy_contribution, outsider, event_manager, token_headers, test_client):
    ...
    db.session.flush()
    resp = test_client.get(f'/api/v1/events/{dummy_event.id}/contributions/{dummy_contribution.id}/acl',
...
def test_contribution_acl_answers_its_manager_before_publication(db, dummy_event, dummy_contribution, dummy_user,
                                                                 token_headers, test_client):
...
def test_attachment_acl_names_the_principals_without_permissions(db, dummy_event, dummy_attachment, outsider,
                                                                 event_manager, token_headers, test_client):
    dummy_attachment.acl.add(outsider)
    db.session.flush()
    resp = test_client.get(f'/api/v1/events/{dummy_event.id}/attachments/{dummy_attachment.id}/acl',
                           headers=token_headers)
    assert resp.status_code == 200
    assert resp.json['results'] == [{'type': 'user', 'identifier': f'User:{outsider.id}', 'name': outsider.name}]
...
def test_attachment_folder_acl(db, dummy_event, dummy_attachment, outsider, event_manager, token_headers, test_client):
    ...
    db.session.flush()
    resp = test_client.get(f'/api/v1/events/{dummy_event.id}/attachment-folders/{folder.id}/acl',
                           headers=token_headers)
    assert resp.status_code == 200
...
def test_attachment_of_another_event_is_not_found(dummy_attachment, create_event, event_manager, token_headers,
                                                  test_client):
    other = create_event()
