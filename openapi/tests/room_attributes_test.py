...
def test_room_attribute_list(dummy_room, room_attributes, token_headers, test_client):
    ...
    assert resp.json['results'] == [{'attribute_id': attributes['notes'].id, 'name': 'notes', 'title': 'notes',
                                     'value': 'Ask at the front desk', 'is_hidden': False}]
...
def test_hidden_room_attributes_are_kept_from_other_users(db, dummy_room, room_attributes, outsider_headers,
                                                          test_client):
...
def test_room_attribute_list_matches_current_api(dummy_room, room_attributes, token_headers, test_client, indico_api,
                                                 same_json_list):
    ...
    same_json_list(new['results'], current, same=ATTRIBUTE_FIELDS,
                   derived={'attribute_id': lambda current: attributes[current['name']].id,
