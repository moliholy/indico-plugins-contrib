...
def create_agreement(db, dummy_event):
    def _create_agreement(person_name, **params):
        ...
        agreement = Agreement(
            ...
            identifier=f'Email:{params["person_email"]}',
            **params,
        )
...
def test_signed_agreement(db, create_agreement, dummy_event, dummy_user, event_manager, token_headers, test_client):
    ...
    agreement = create_agreement(
        'Alice Smith',
        state=AgreementState.accepted_on_behalf,
        user=dummy_user,
        signed_dt=signed_dt,
        reason='Signed by phone',
        attachment_filename='release.pdf',
        data={'speaker_id': 7},
    )
    db.session.flush()
...
def test_agreement_list_filtered_by_type(
    dummy_agreement, create_agreement, dummy_event, event_manager, token_headers, test_client
):
    create_agreement('Alice Smith', type='other-release')
    resp = test_client.get(
        f'/api/v1/events/{dummy_event.id}/agreements?type=cern-speaker-release', headers=token_headers
    )
    assert [a['id'] for a in resp.json['results']] == [dummy_agreement.id]


def test_agreement_does_not_expose_the_signing_token(
    dummy_agreement, dummy_event, event_manager, token_headers, test_client
):
    resp = test_client.get(f'/api/v1/events/{dummy_event.id}/agreements/{dummy_agreement.id}', headers=token_headers)
...
def test_agreements_are_manager_only(dummy_agreement, dummy_event, outsider_headers, test_client):
    resp = test_client.get(f'/api/v1/events/{dummy_event.id}/agreements/{dummy_agreement.id}', headers=outsider_headers)
    assert resp.status_code == 403
    resp = test_client.get(f'/api/v1/events/{dummy_event.id}/agreements', headers=outsider_headers)
    assert resp.status_code == 403


def test_agreement_of_another_event_is_not_found(
    dummy_agreement, create_event, dummy_user, db, token_headers, test_client
):
    other = create_event()
