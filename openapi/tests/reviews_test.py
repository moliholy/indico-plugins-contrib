...
def abstract_question(db, dummy_event):
    question = AbstractReviewQuestion(event=dummy_event, field_type='rating', title='Originality',
                                      description='How novel the work is', field_data={'min': 0, 'max': 5})
...
def create_abstract_review(db, dummy_abstract, dummy_track, abstract_question):
    def _create(user, value=3):
        ...
        review = AbstractReview(abstract=dummy_abstract, user=user, track=dummy_track,
                                proposed_action=AbstractAction.accept, comment='Worth accepting')
...
def paper_question(db, dummy_event):
    question = PaperReviewQuestion(event=dummy_event, type=PaperReviewType.content, field_type='rating',
                                   title='Clarity', description='How readable the paper is',
...
def create_paper_review(db, dummy_paper_revision, paper_question):
    def _create(user, value=4):
        review = PaperReview(revision=dummy_paper_revision, user=user, type=PaperReviewType.content,
                             proposed_action=PaperAction.accept, comment='Ready as it is')
...
def test_abstract_reviews_are_served_to_a_judge(dummy_event, dummy_abstract, create_abstract_review, reviewer,
                                                event_manager, token_headers, test_client):
    review = create_abstract_review(reviewer)
    resp = test_client.get(f'/api/v1/events/{dummy_event.id}/abstracts/{dummy_abstract.id}/reviews',
...
def test_abstract_review_carries_its_ratings(dummy_event, dummy_abstract, create_abstract_review, reviewer,
                                             abstract_question, event_manager, token_headers, test_client):
...
def test_abstract_review_proposing_a_merge_names_the_other_abstract(db, dummy_event, dummy_abstract, dummy_track,
                                                                   create_abstract, reviewer, event_manager,
                                                                   token_headers, test_client):
    other = create_abstract(dummy_event, 'Another abstract', friendly_id=315, submitter=reviewer)
    dummy_abstract.reviewed_for_tracks = {dummy_track}
    db.session.add(AbstractReview(abstract=dummy_abstract, user=reviewer, track=dummy_track,
                                  proposed_action=AbstractAction.merge, proposed_related_abstract=other))
    db.session.flush()
    resp = test_client.get(f'/api/v1/events/{dummy_event.id}/abstracts/{dummy_abstract.id}/reviews',
                           headers=token_headers)
...
def test_abstract_review_is_hidden_from_the_submitter(dummy_event, dummy_abstract, create_abstract_review, reviewer,
                                                      token_headers, test_client):
...
def test_abstract_review_is_served_to_its_author(dummy_event, dummy_abstract, create_abstract_review, dummy_user,
                                                 token_headers, test_client):
...
def test_abstract_reviews_need_access_to_the_abstract(dummy_event, dummy_abstract, create_abstract_review, reviewer,
...
def test_abstract_comments_honour_their_visibility(db, dummy_event, dummy_abstract, reviewer, token_headers,
                                                   test_client):
    hidden = AbstractComment(abstract=dummy_abstract, user=reviewer, text='Only for judges',
                             visibility=AbstractCommentVisibility.judges)
    shown = AbstractComment(abstract=dummy_abstract, user=reviewer, text='Please clarify section 2',
                            visibility=AbstractCommentVisibility.contributors)
    db.session.add_all([hidden, shown])
    db.session.flush()
    resp = test_client.get(f'/api/v1/events/{dummy_event.id}/abstracts/{dummy_abstract.id}/comments',
                           headers=token_headers)
    assert resp.status_code == 200
...
def test_abstract_comment_is_served_to_its_author(db, dummy_event, dummy_abstract, dummy_track, reviewer, test_client,
    ...
    comment = AbstractComment(abstract=dummy_abstract, user=reviewer, text='Only for judges',
                              visibility=AbstractCommentVisibility.judges)
...
def test_abstract_review_questions_are_served_to_the_abstract_managers(dummy_event, abstract_question, event_manager,
                                                                      token_headers, test_client):
...
def test_abstract_review_questions_are_forbidden_without_management(dummy_event, abstract_question, token_headers,
                                                                    test_client):
    resp = test_client.get(f'/api/v1/events/{dummy_event.id}/abstract-review-questions', headers=token_headers)
    assert resp.status_code == 403


def test_paper_reviews_are_served_to_a_judge(dummy_event, dummy_contribution, dummy_paper_revision,
                                             create_paper_review, reviewer, event_manager, token_headers, test_client):
    ...
    resp = test_client.get(f'/api/v1/events/{dummy_event.id}/contributions/{dummy_contribution.id}'
                           f'/paper/revisions/{dummy_paper_revision.id}/reviews', headers=token_headers)
    assert resp.status_code == 200
    assert [r['id'] for r in resp.json['results']] == [review.id]
    assert resp.json['results'][0]['user']['id'] == reviewer.id
...
def test_paper_review_carries_its_ratings(dummy_event, dummy_contribution, dummy_paper_revision, create_paper_review,
                                          reviewer, paper_question, event_manager, token_headers, test_client):
...
def test_paper_review_is_hidden_from_the_submitter(dummy_event, dummy_contribution, dummy_paper_revision,
                                                   create_paper_review, reviewer, token_headers, test_client):
...
def test_paper_reviews_need_access_to_the_paper(dummy_event, dummy_contribution, dummy_paper_revision,
                                                create_paper_review, reviewer, outsider_headers, test_client):
    create_paper_review(reviewer)
    resp = test_client.get(f'/api/v1/events/{dummy_event.id}/contributions/{dummy_contribution.id}'
                           f'/paper/revisions/{dummy_paper_revision.id}/reviews', headers=outsider_headers)
    assert resp.status_code == 403


def test_paper_reviews_of_an_unknown_revision_are_not_found(dummy_event, dummy_contribution, dummy_paper_revision,
                                                            event_manager, token_headers, test_client):
    resp = test_client.get(f'/api/v1/events/{dummy_event.id}/contributions/{dummy_contribution.id}'
                           f'/paper/revisions/{dummy_paper_revision.id + 100}/reviews', headers=token_headers)
    assert resp.status_code == 404


def test_paper_comments_honour_their_visibility(db, dummy_event, dummy_contribution, dummy_paper_revision, reviewer,
                                                token_headers, test_client):
    hidden = PaperReviewComment(paper_revision=dummy_paper_revision, user=reviewer, text='Only for judges',
                                visibility=PaperCommentVisibility.judges)
    shown = PaperReviewComment(paper_revision=dummy_paper_revision, user=reviewer, text='Check the references',
                               visibility=PaperCommentVisibility.contributors)
    db.session.add_all([hidden, shown])
    db.session.flush()
    resp = test_client.get(f'/api/v1/events/{dummy_event.id}/contributions/{dummy_contribution.id}'
                           f'/paper/revisions/{dummy_paper_revision.id}/comments', headers=token_headers)
    assert resp.status_code == 200
    assert [c['id'] for c in resp.json['results']] == [shown.id]
...
def test_paper_review_questions_are_served_to_the_paper_managers(dummy_event, paper_question, event_manager,
                                                                 token_headers, test_client):
...
def test_paper_review_questions_are_forbidden_without_management(dummy_event, paper_question, token_headers,
                                                                 test_client):
    resp = test_client.get(f'/api/v1/events/{dummy_event.id}/paper-review-questions', headers=token_headers)
    assert resp.status_code == 403


ABSTRACT_REVIEW_FIELDS = ('id', 'user', 'track', 'comment', 'proposed_action', 'proposed_contrib_type',
                          'proposed_related_abstract', 'proposed_tracks', 'created_dt', 'modified_dt')
...
def as_rated_questions(ratings):
    ...
    return [{'question': {key: value for key, value in rating['question'].items() if key != 'no_score'},
             'value': rating['value']}
...
def as_review_score(current):
    ...
    """
    values = [rating['value'] for rating in current['ratings']
...
ABSTRACT_REVIEW_MAPPINGS = {'same': ABSTRACT_REVIEW_FIELDS,
                            'renamed': {'ratings': ('ratings', as_question_ids)},
                            'derived': {'score': as_review_score}}
PAPER_REVIEW_MAPPINGS = {'same': PAPER_REVIEW_FIELDS,
                         'renamed': {'ratings': ('ratings', as_rated_questions)},
                         'derived': {'type': as_review_type, 'proposed_action': as_action_name,
                                     'score': as_paper_score}}
PAPER_COMMENT_MAPPINGS = {'same': tuple(field for field in COMMENT_FIELDS if field != 'visibility'),
                          'derived': {'visibility': as_visibility_name}}
PAPER_QUESTION_MAPPINGS = {'same': PAPER_QUESTION_FIELDS, 'derived': {'no_score': as_no_score}}


def test_abstract_reviews_match_the_current_api(dummy_event, dummy_abstract, create_abstract_review, reviewer,
    ...
    ours = test_client.get(f'/api/v1/events/{dummy_event.id}/abstracts/{dummy_abstract.id}/reviews',
                           headers=token_headers).json['results']
    same_json_list(ours, theirs, **ABSTRACT_REVIEW_MAPPINGS)


def test_abstract_comments_match_the_current_api(db, dummy_event, dummy_abstract, reviewer, event_manager,
                                                 token_headers, test_client, indico_api, same_json_list):
    db.session.add(AbstractComment(abstract=dummy_abstract, user=reviewer, text='Please clarify section 2',
                                   visibility=AbstractCommentVisibility.contributors))
    db.session.flush()
    current = indico_api(f'/event/{dummy_event.id}/manage/abstracts/abstracts.json')['abstracts']
    theirs = next(abstract for abstract in current if abstract['id'] == dummy_abstract.id)['comments']
    ours = test_client.get(f'/api/v1/events/{dummy_event.id}/abstracts/{dummy_abstract.id}/comments',
                           headers=token_headers).json['results']
    same_json_list(ours, theirs, same=COMMENT_FIELDS)


def test_paper_reviews_match_the_current_api(dummy_event, dummy_contribution, dummy_paper_revision,
                                             create_paper_review, reviewer, event_manager, token_headers,
    ...
    ours = test_client.get(f'/api/v1/events/{dummy_event.id}/contributions/{dummy_contribution.id}'
                           f'/paper/revisions/{dummy_paper_revision.id}/reviews',
                           headers=token_headers).json['results']
    same_json_list(ours, theirs, **PAPER_REVIEW_MAPPINGS)


def test_paper_comments_match_the_current_api(db, dummy_event, dummy_contribution, dummy_paper_revision, reviewer,
                                              event_manager, token_headers, test_client, indico_api,
                                              same_json_list):
    db.session.add(PaperReviewComment(paper_revision=dummy_paper_revision, user=reviewer,
                                      text='Check the references', visibility=PaperCommentVisibility.contributors))
    db.session.flush()
    current = indico_api(f'/event/{dummy_event.id}/manage/papers/assignment-list/export-json')['papers']
    paper = next(p for p in current if p['contribution']['id'] == dummy_contribution.id)
...
def test_paper_review_questions_match_the_current_api(dummy_event, paper_question, event_manager, token_headers,
                                                      test_client, indico_api, same_json_list):
