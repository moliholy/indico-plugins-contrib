...
class ServiceRequestListArgs(ListArgs):
    ...
    state = fields.Enum(
        RequestState, load_default=None, metadata={'description': 'Only list the requests in this state.'}
    )
...
class RHServiceRequest(ServiceRequestMixin, RHManageEventBase):
    def _process_args(self):
        ...
        self.request = self._request_query().filter(Request.id == request.view_args['request_id']).first_or_404()
...
ENDPOINTS = [
    Endpoint(
        rule='/events/<int:event_id>/requests',
        name='requests',
        rh=RHServiceRequestList,
        schema=ServiceRequestSchema,
        many=True,
        summary='List the services an event asked for',
        tag='Service requests',
    ),
    Endpoint(
        rule='/events/<int:event_id>/requests/<int:request_id>',
        name='request',
        rh=RHServiceRequest,
        schema=ServiceRequestSchema,
        summary='Details of one service an event asked for',
        tag='Service requests',
    ),
]
