...
class LogListArgs(ListArgs):
    ...
    q = fields.String(load_default=None,
                      metadata={'description': 'Only list entries matching this text in their module, type or '
                                               'summary, in the name of the user who performed the action, or in '
                                               'the addresses, subject and body of a sent email.'})
...
class RHEventLogEntryList(LogMixin, RHListBase, RHManageEventBase):
    ...
    def _query(self, realm, q, registration_id):
        ...
        if q:
            # the same columns the log interface searches, matched the same way
            query = query.filter(db.or_(_contains(EventLogEntry.module, q),
                                        _contains(EventLogEntry.type, q),
                                        _contains(EventLogEntry.summary, q),
                                        _contains(db.m.User.first_name + ' ' + db.m.User.last_name, q),
                                        _contains(EventLogEntry.data['body'].astext, q),
                                        _contains(EventLogEntry.data['subject'].astext, q),
                                        _contains(EventLogEntry.data['from'].astext, q),
                                        _contains(EventLogEntry.data['to'].astext, q),
                                        _contains(EventLogEntry.data['cc'].astext, q))
                                 ).outerjoin(db.m.User, db.m.User.id == EventLogEntry.user_id)
