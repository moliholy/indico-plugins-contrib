...
class CategoryLogEntrySchema(LogEntrySchema):
    ...
    class Meta(LogEntrySchema.Meta):
        ...
        descriptions = {
            **LogEntrySchema.Meta.descriptions,
...
SEARCHED_TEXT = (
    'Only list entries matching this text in their module, type or summary, in the name of the user '
...
class LogListArgs(ListArgs):
    ...
    registration_id = fields.Integer(
        load_default=None,
        metadata={
            'description': 'Only list entries about this registration. A caller '
...
class LogMixin:
    ...
    def _entry_query(self):
        return EventLogEntry.query.with_parent(self.event).order_by(
            EventLogEntry.logged_dt.desc(), EventLogEntry.id.desc()
        )
...
class CategoryLogMixin:
    ...
    def _entry_query(self):
        return self.category.log_entries.order_by(CategoryLogEntry.logged_dt.desc(), CategoryLogEntry.id.desc())
...
class RHCategoryLogEntry(CategoryLogMixin, RHManageCategoryBase):
    def _process_args(self):
        ...
        self.entry = self._entry_query().filter(CategoryLogEntry.id == request.view_args['entry_id']).first_or_404()
...
ENDPOINTS = [
    Endpoint(
        rule='/events/<int:event_id>/logs',
        name='logs',
        rh=RHEventLogEntryList,
        schema=LogEntrySchema,
        many=True,
        summary='List the log entries of an event',
        tag='Logs',
    ),
    Endpoint(
        rule='/events/<int:event_id>/logs/<int:entry_id>',
        name='log_entry',
        rh=RHEventLogEntry,
        schema=LogEntrySchema,
        summary='Details of one log entry of an event',
        tag='Logs',
    ),
    Endpoint(
        rule='/categories/<int:category_id>/logs',
        name='category_logs',
        rh=RHCategoryLogEntryList,
        schema=CategoryLogEntrySchema,
        many=True,
        summary='List the log entries of a category',
        tag='Logs',
    ),
    Endpoint(
        rule='/categories/<int:category_id>/logs/<int:entry_id>',
        name='category_log_entry',
        rh=RHCategoryLogEntry,
        schema=CategoryLogEntrySchema,
        summary='Details of one log entry of a category',
        tag='Logs',
    ),
]
