...
class ReminderSchema(DescribedFieldsMixin, mm.SQLAlchemyAutoSchema):
    ...
    class Meta:
        model = EventReminder
        fields = ('id', 'reminder_type', 'creator', 'created_dt', 'scheduled_dt', 'event_start_delta',
                  'event_end_delta', 'is_sent', 'recipients', 'send_to_participants', 'forms', 'tags', 'all_tags',
                  'send_to_speakers', 'reply_to_address', 'subject', 'message', 'render_mode', 'include_summary',
                  'include_description', 'attach_ical')
        descriptions = {
...
class ReminderMixin:
    ...
    def _reminder_query(self):
        return (EventReminder.query.with_parent(self.event)
...
class RHReminder(ReminderMixin, RHProtectedEventBase):
    def _process_args(self):
        ...
        self.reminder = (self._reminder_query()
                         .filter(EventReminder.id == request.view_args['reminder_id']).first_or_404())
